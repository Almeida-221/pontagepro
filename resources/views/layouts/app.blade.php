...
<body class="bg-gray-50 text-gray-800 antialiased">
    ...
    @php
        ...
        $ftFb      = \App\Models\SiteSetting::get('facebook_url');
        $ftTk      = \App\Models\SiteSetting::get('tiktok_url');
        $ftYt      = \App\Models\SiteSetting::get('youtube_url');
    @endphp
    <footer class="bg-gray-900 text-gray-400 mt-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            @if($ftFb || $ftTk || $ftYt)
            <div class="border-t border-gray-800 mt-8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
                <p>&copy; {{ date('Y') }} {{ $ftName }}. Tous droits réservés.</p>
                <div class="flex items-center gap-4">
                    @if($ftFb)
                    <a href="{{ $ftFb }}" target="_blank" rel="noopener" aria-label="Facebook"
                       class="text-gray-400 hover:text-white transition">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                    @endif
                    @if($ftTk)
                    <a href="{{ $ftTk }}" target="_blank" rel="noopener" aria-label="TikTok"
                       class="text-gray-400 hover:text-white transition">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.27 8.27 0 004.84 1.55V6.79a4.85 4.85 0 01-1.07-.1z"/></svg>
                    </a>
                    @endif
                    @if($ftYt)
                    <a href="{{ $ftYt }}" target="_blank" rel="noopener" aria-label="YouTube"
